 ...
 */
@SupportedAnnotationTypes(RouterConsts.PARAM_ANNOTATION_TYPE)
...
public class InjectParamProcessor extends AbstractProcessor {
    ...
    private void generate() throws IllegalAccessException, IOException {
        ...
        for (Map.Entry<TypeElement, List<Element>> entry : mClzAndParams.entrySet()) {
            ...
            String fileName = simpleName + RouterConsts.PARAM_CLASS_SUFFIX;

            // validate
            boolean isActivity;
            if (isSubtype(parent, RouterConsts.ACTIVITY_FULL_NAME)) {
                isActivity = true;
            } else if (isSubtype(parent, RouterConsts.FRAGMENT_V4_FULL_NAME) || isSubtype(parent, RouterConsts.FRAGMENT_FULL_NAME)) {
                isActivity = false;
            } else {
            ...
            MethodSpec.Builder injectMethodBuilder = MethodSpec.methodBuilder(RouterConsts.METHOD_INJECT)
            ...
            injectMethodBuilder.addStatement("$T $L = ($T) $L",
                    ClassName.get((Type) parent), TARGET, ClassName.get(parent), OBJ);
            ...
            TypeElement interfaceType = processingEnv.getElementUtils().getTypeElement(RouterConsts.PARAM_INJECTOR_FULL_NAME);
