...
public class VerticalBannerView extends LinearLayout implements BaseBannerAdapter.OnDataChangedListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mGap = array.getInteger(R.styleable.VerticalBannerView_vertical_gap,mGap);
        mAnimDuration = array.getInteger(R.styleable.VerticalBannerView_vertical_duration,mAnimDuration);
        itemNum = array.getInteger(R.styleable.VerticalBannerView_vertical_item_num,itemNum);
    ...
    private void setupAdapter() {
        ...
        if(mAdapter.getCount() == 1){
            ...
            mAdapter.setItem(mFirstView,mAdapter.getItemList(itemNum,mPosition));
            addView(mFirstView);
        }else{
            mFirstView = mAdapter.getView(this);
            mSecondView = mAdapter.getView(this);
            mAdapter.setItem(mFirstView,mAdapter.getItemList(itemNum,mPosition));
            mAdapter.setItem(mSecondView,mAdapter.getItemList(itemNum,++mPosition));
            addView(mFirstView);
            addView(mSecondView);
            isStarted = false;
        }
    ...
    private void performSwitch(){
        ...
        set.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mAdapter.setItem(removedView,mAdapter.getItemList(itemNum,++mPosition));
