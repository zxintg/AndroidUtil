...
import com.zxin.router.annotation.InjectParam;
 ...
 */
@SupportedAnnotationTypes(RouterConsts.ROUTE_ANNOTATION_TYPE)
...
public class RouteProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        if (mModuleName != null) {
            ...
            throw new RuntimeException(String.format("No option `%s` passed to Route annotation processor.", RouterConsts.OPTION_MODULE_NAME));
        }
    ...
    private boolean validateClass(TypeElement typeElement) {
        if (!isSubtype(typeElement, RouterConsts.ACTIVITY_FULL_NAME) && !isSubtype(typeElement, RouterConsts.FRAGMENT_V4_FULL_NAME)
                && !isSubtype(typeElement, RouterConsts.FRAGMENT_FULL_NAME)) {
    ...
    private void generateRouteTable(String moduleName, Set<TypeElement> elements) {
        ...
        MethodSpec.Builder methodHandle = MethodSpec.methodBuilder(RouterConsts.METHOD_HANDLE)
        ...
        TypeElement interfaceType = processingEnv.getElementUtils().getTypeElement(RouterConsts.ROUTE_TABLE_FULL_NAME);
        TypeSpec type = TypeSpec.classBuilder(capitalize(moduleName) + RouterConsts.ROUTE_TABLE)
    ...
    private void generateTargetInterceptors(String moduleName, Set<TypeElement> elements) {
        ...
        TypeElement interfaceType = processingEnv.getElementUtils().getTypeElement(RouterConsts.TARGET_INTERCEPTORS_FULL_NAME);
        TypeSpec type = TypeSpec.classBuilder(capitalize(moduleName) + RouterConsts.TARGET_INTERCEPTORS)
                .addSuperinterface(ClassName.get(interfaceType))
                .addModifiers(Modifier.PUBLIC)
                .addMethod(methodHandle.build())
                .addJavadoc(RouterConsts.CLASS_JAVA_DOC)
