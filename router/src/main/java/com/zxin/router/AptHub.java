...
import com.zxin.router.template.ParamInjector;
 ...
 * Hub for 'apt' classes.
 * <p>
 * Created by zxin on 2017/3/13.
 */
class AptHub {
    private static final String PACKAGE_NAME = "com.zxin.router";
    ...
    synchronized static void registerModules(String... modules) {
        if (modules == null || modules.length == 0) {
            ...
            for (String module : modules) {
                try {
                    routeTableName = PACKAGE_NAME + DOT + capitalize(module) + ROUTE_TABLE;//APT动态生成
