...
import com.zxin.zxinlib.gen.MeiZiCollectDao;
import com.zxin.zxinlib.gen.MeiZiVideoDao;
...
public class MeiZiVideoDaoUtil {
    ...
    private static MeiZiVideoDaoUtil meiZiVideoDaoUtil;
    private static MeiZiVideoDao meiZiVideoDao;
    private static MeiZiCollectDao meiZiCollectDao;
    public static MeiZiVideoDaoUtil getInstance(){
        if (meiZiVideoDaoUtil==null){
            meiZiVideoDaoUtil = new MeiZiVideoDaoUtil();
        }
    ...
    public boolean addMeiZiVideoJsonArray(String json) {
        Gson gson = new Gson();
        List<MeiZiVideo> collectList = gson.fromJson(json, new TypeToken<List<MeiZiVideo>>() {}.getType());
        if (collectList==null||collectList.isEmpty()){
            ToastUtil.showShort("视频数据有问题，请检查！");
            return false;
        }
    ...
    public boolean addMeiZiCollectJsonArray(String json) {
        Gson gson = new Gson();
        List<MeiZiCollect> collectList = gson.fromJson(json, new TypeToken<List<MeiZiCollect>>() {}.getType());
        if (collectList==null||collectList.isEmpty()){
            ToastUtil.showShort("收藏数据有问题，请检查！");
            return false;
        }
